...
API_TOKEN = 'Your API Token Goes Here'  # Replace with your bot's API token
...
def load_accounts():
    with open('accounts.json', 'r', encoding='utf-8') as f:  # Specify the encoding here
        return json.load(f)

accounts_data = load_accounts()  # Load accounts at startup
...
# Define the admin  chat ID for checking admins
...
def check_joined(chat_id):
    channels_to_check = ["@abir_x_official", "@abir_x_official_developer"]
...
# Handle service selection in redeem section
@bot.message_handler(func=lambda message: message.text.split(" (")[0] in service_points.keys())
def service_handler(message):
    chat_id = message.chat.id
    service = message.text.split(" (")[0]  # Extract the service name without points
    required_points = service_points.get(service)
    ...
    print(f"Available services in accounts_data: {list(accounts_data.keys())}")
    
    # Check if user has enough points
    if user['balance'] >= required_points:
        # Check if accounts are available for the selected service
        if accounts_data.get(service) and len(accounts_data[service]) > 0:
            # Deduct points from user balance
            user['balance'] -= required_points

            # Provide the first account in the list to the user
            account = accounts_data[service].pop(0)
            bot.send_message(chat_id, f"Here is your {service} account:\n{account}")

            # Save the updated accounts back to accounts.json
            save_accounts(accounts_data)
        else:
            bot.send_message(chat_id, f"⚠️ Sorry, no {service} accounts are currently available.")
    else:
        bot.send_message(chat_id, f"⚠️ You need {required_points} points to redeem {service}. You currently have {user['balance']} points.")


# Handle the Refer button
