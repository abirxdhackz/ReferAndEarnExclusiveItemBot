...
API_TOKEN = '7858047672:AAGzzgnkEEvjw7hj2Ld4rq6CA0vG3aiF3OY'  # Replace with your bot's API token
...
# Define VPN sub-options with their required points
vpn_options = {
    "🔒Nord VPN": 6,
    "🔒Express VPN": 6,
    "🔒Surfshark VPN": 6,
    "🔒PIA VPN": 6,
    "🔒Hotspot Shield VPN": 6
}

# Update the service handler to handle VPN selection
@bot.message_handler(func=lambda message: message.text.split(" (")[0] in service_points.keys() or message.text.split(" (")[0] in vpn_options.keys())
def service_handler(message):
    chat_id = message.chat.id
    service = message.text.split(" (")[0]
    user = user_data.get(chat_id, {'balance': 0})

    # Check if the main "🔒VPN" option was selected
    if service == "🔒VPN":
        # Display VPN options in a 3x3 grid layout
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        vpn_buttons = [f"{vpn} ({points} points)" for vpn, points in vpn_options.items()]

        # Arrange VPN buttons in rows of 3
        for i in range(0, len(vpn_buttons), 3):
            markup.add(*vpn_buttons[i:i + 3])

        markup.add("🔙 Back to Menu")  # Add a back button to return to the main menu
        bot.send_message(chat_id, "Select a specific VPN service:", reply_markup=markup)

    # Check if a specific VPN option was selected
    elif service in vpn_options:
        required_points = vpn_options[service]
        
        # Check if the user has enough points for the selected VPN service
        if user['balance'] >= required_points:
            # Deduct points and forward the request to the admin group
    ...
    # Handle selection of other services outside of VPN options
    else:
        required_points = service_points.get(service, 0)
        
        # Check if the user has enough points for the selected service
        if user['balance'] >= required_points:
            user['balance'] -= required_points
            forwarded_message_text = f"User ID: {chat_id}\nUser requested: {service}"
            sent_message = bot.send_message(GROUP_CHAT_ID, forwarded_message_text)
...
def main_menu(chat_id):
    ...
    join_buttons = [
        types.InlineKeyboardButton("Dev Channel", url="https://t.me/abir_x_official"),
